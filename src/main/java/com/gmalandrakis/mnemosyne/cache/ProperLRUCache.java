...
public class ProperLRUCache<K, ID, T> extends AbstractGenericCache<K, ID, T> {
    final ConcurrentHashMap<ID, Integer> numberOfUsesById = new ConcurrentHashMap<ID, Integer>();
    private final Boolean lock = true;

    public ProperLRUCache(CacheParameters cacheParameters, ValuePool poolService) {
    ...
    public void evict() {
        ...
        while (numberOfUsesById.size() >= this.actualCapacity) {
            var it = ((LinkedHashMap<K, IdWrapper<ID>>) keyIdMapper).entrySet().iterator();
